...
Page({
  ...
  getCollectList() {
    let that = this;
    wx.request({
      url: app.globalData.domain + '/api/frontend/news/collect/lists',
      method: 'GET',
      data: {
      ...
      success(res) {
        if (res.data.msg_code === 100000) {
          that.setData({
            list: res.data.response.data.map(item => {
              if(item.news){
                item.news.tags = [
  ...
  voteNews(event){
    let id = event.currentTarget.dataset.new.news_id;
    let text_type = event.currentTarget.dataset.new.news.text_type;
    let that = this;
    wx.request({
  ...
  deletenews(event){
    ...
    let that = this;
    wx.request({
      url: app.globalData.domain + '/api/frontend/news/collect/delete',
      method: 'POST',
      data: {
        token: wx.getStorageSync('userInfo').token,
        news_id: id
      ...
      success(res) {
        if (res.data.msg_code === 100000) {
          ...
          that.getCollectList();
          utils.count_cancel(id,"collect",text_type)
        } else {
          wx.showToast({
            title: res.data.message,
            icon: 'none',
